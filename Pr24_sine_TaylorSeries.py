...
class TaylorSeries(Scene):
    def construct(self):
        ...
        eq.move_to(UP*3.2)
        eq1=MathTex(r"sin(x)", r"\approx", r"x", color=BLUE)
        eq1.move_to(UP*3.2)
        eq3=MathTex(r"sin(x)", r"\approx", r"x - \dfrac{x^3}{3!}", color=RED)
        eq3.move_to(UP*3.2)
        eq5=MathTex(r"sin(x)", r"\approx", r"x - \dfrac{x^3}{3!} + \dfrac{x^5}{5!}", color=GREEN)
        eq5.move_to(UP*3.2)
        eq7=MathTex(r"sin(x)", r"\approx", r"x - \dfrac{x^3}{3!} + \dfrac{x^5}{5!} - \dfrac{x^7}{7!}", color=PINK)
        eq7.move_to(UP*3.2)
        eq9=MathTex(r"sin(x)", r"\approx", r"x - \dfrac{x^3}{3!} + \dfrac{x^5}{5!} - \dfrac{x^7}{7!} + \dfrac{x^9}{9!}",color=TEAL)
        eq9.move_to(UP*3.2)
        ...
        sl=axes.get_graph_label( sine, r"\sin(x)", x_val=4, color=WHITE, direction=UP)
            ...
            # Taylor Polynomials
        p1=axes.plot(lambda x: x, color=BLUE, x_range=[-6, 6])

        p3=axes.plot(lambda x: x - x**3 /6, color=RED, x_range=[-6, 6])

        p5=axes.plot(lambda x: x - x**3/6 + x**5/120, color=GREEN, x_range=[-6, 6])

        p7=axes.plot(lambda x: x - x**3/6 + x**5/120 - x**7/5040, color=PINK, x_range=[-6, 6])

        p9=axes.plot(lambda x: x - x**3/6 + x**5/120 - x**7/5040 + x**9/362880, color=TEAL, x_range=[-6, 6])
        ...
        # manim Pr24_sine_TaylorSeries.py Manim24 --renderer=opengl -p
        # manim -pqk Pr24_sine_TaylorSeries.py Manim24
        # manim -pqh Pr24_sine_TaylorSeries.py Manim24
